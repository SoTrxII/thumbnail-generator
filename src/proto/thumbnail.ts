...
export namespace thumbnail {
    export class ThumbnailRequest extends pb_1.Message {
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            ...
            if (this.episodeTitle.length)
                writer.writeString(3, this.episodeTitle);
            if (this.episodeIndex != 0)
                writer.writeInt32(4, this.episodeIndex);
            if (this.backgroundUrl.length)
                writer.writeString(5, this.backgroundUrl);
            if (this.logoUrl.length)
                writer.writeString(6, this.logoUrl);
            if (!w)
    ...
    export class ThumbnailResponse extends pb_1.Message {
        ...
        serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
            const writer = w || new pb_1.BinaryWriter();
            if (this.thumbnailKey.length)
                writer.writeString(1, this.thumbnailKey);
            if (!w)
                return writer.getResultBuffer();
        }
        static deserialize(bytes: Uint8Array | pb_1.BinaryReader): ThumbnailResponse {
            const reader = bytes instanceof pb_1.BinaryReader ? bytes : new pb_1.BinaryReader(bytes), message = new ThumbnailResponse();
            while (reader.nextField()) {
                if (reader.isEndGroup())
                    break;
                switch (reader.getFieldNumber()) {
    ...
    export abstract class UnimplementedThumbnailService {
        ...
        abstract CreateThumbnail(call: grpc_1.ServerUnaryCall<ThumbnailRequest, ThumbnailResponse>, callback: grpc_1.sendUnaryData<ThumbnailResponse>): void;
    }
    export class ThumbnailClient extends grpc_1.makeGenericClientConstructor(UnimplementedThumbnailService.definition, "Thumbnail", {}) {
        constructor(address: string, credentials: grpc_1.ChannelCredentials, options?: Partial<grpc_1.ChannelOptions>) {
            super(address, credentials, options);
        }
        CreateThumbnail: GrpcUnaryServiceInterface<ThumbnailRequest, ThumbnailResponse> = (message: ThumbnailRequest, metadata: grpc_1.Metadata | grpc_1.CallOptions | grpc_1.requestCallback<ThumbnailResponse>, options?: grpc_1.CallOptions | grpc_1.requestCallback<ThumbnailResponse>, callback?: grpc_1.requestCallback<ThumbnailResponse>): grpc_1.ClientUnaryCall => {
